...
- **changelog** - update `CHANGELOG.md` with entries for the changes.
  ...
  files under `itunes_clear_orphaned/`, `src/`, or
...
For example, if a commit contains `itunes_clear_orphaned/src/index.ts`, and `CHANGELOG.md`,
the component is determined by the source files only. `CHANGELOG.md` is simply staged alongside
them.
